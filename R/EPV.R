...
th.new[j+1]<-eapEst(it=it.new,x=c(x,j),model=model,priorDist=priorDist,priorPar=priorPar,lower=parInt[1],upper=parInt[2],nqp=parInt[3])
se.new[j+1]<-(eapSem(th.new[j+1],it=it.new,x=c(x,j),model=model,priorDist=priorDist,priorPar=priorPar,lower=parInt[1],upper=parInt[2],nqp=parInt[3]))^2
}
