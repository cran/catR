nextItem<- function (itemBank, model=NULL, theta=0, out = NULL, x = NULL, criterion = "MFI",  
                     method = "BM", priorDist = "norm", priorPar = c(0, 1), D = 1, 
                     range = c(-4, 4), parInt = c(-4, 4, 33), infoType = "observed", randomesque = 1, 
                     rule="length", thr=20, SETH=NULL, AP=1, nAvailable=NULL, maxItems=50, 
...
if (!is.null(model)){
mod<-switch(model,GRM=1,MGRM=2,PCM=3,GPCM=4,RSM=5,NRM=6)
if (is.null(mod)) stop("invalid 'model' type!",call.=FALSE)
}
  if (is.null(cbControl)) 
    OUT <- out
  else {
    if (is.null(cbGroup)) stop("'cbGroup' argument must be provided for content balancing!",call.=FALSE)
    ...
    if (is.null(out)) 
      ...
      empProp <- NULL
      for (i in 1:nrGroup) 
        empProp[i] <- length(out[cbGroup[out]==cbControl$names[i]])
  ...
  if (crit == "MFI") {
    ...
    for (i in 1:length(keepRank)) 
      keep[i] <- which(ranks==keepRank[i])
    select <- ifelse(length(keep) == 1, keep, sample(c(keep), 1))
    res <- list(item = select, par = itemBank[select,], info = info[select], criterion = criterion, 
                randomesque = randomesque)
  }

  if (crit == "bOpt") {
    if (!is.null(model)) stop("bOpt's rule cannot be considered with polytomous items",call.=FALSE)
    items <- rep(1, nrow(itemBank)) 
    items[OUT] <- 0
    ...
    res <- list(item = select, par = itemBank[select,], info = distance[select], criterion = criterion, 
                randomesque = randomesque)
  }
  ...
  if (crit == "MLWI" | crit == "MPWI") {
    ...
    for (i in 1:nrow(itemBank)) {
      if (ITEMS[i] == 1) 
        likInfo[i] <- MWI(itemBank, i, x, it.given=par, model=model, type = criterion, 
    ...
    select <- ifelse(length(keep) == 1, keep, sample(keep, 1))
    res <- list(item = select, par = itemBank[select,], info = likInfo[select], 
  ...
  if (crit == "KL" | crit == "KLP") {
    ...
    LL <- function(th, r, param, model) {
      prob <- Pi(th, param, model = model)$Pi
      res <- 1
      for (i in 1:length(r)) res <- res * prob[i, r[i] + 1]
        return(res)
    }
    if (is.null(model)) LF <- sapply(X,L,x,par)
      else LF<-sapply(X,LL,x,par,model=model)
    for (i in 1:nrow(itemBank)) {
      if (ITEMS[i] == 1) 
        klvalue[i] <- KL(itemBank, i, x, it.given=par, model=model, theta=theta, type = criterion, 
                         lower = parInt[1], upper = parInt[2], nqp = parInt[3], 
                         priorDist = priorDist, priorPar = priorPar, lik = LF, X=X)
    }
    klVal <- sort(klvalue, decreasing = TRUE)[min(c(randomesque, sum(ITEMS)))]
    keep <- (1:length(ITEMS))[klvalue >= klVal]
    select <- ifelse(length(keep) == 1, keep, sample(keep, 1))
    res <- list(item = select, par = itemBank[select,], info = klvalue[select], 
                criterion = criterion, randomesque = randomesque)
  }  
    if (crit == "MEI") {
    items <- rep(1, nrow(itemBank))
    ...
    for (i in 1:length(items)) {
      if (items[i] > 0) 
        infos[i] <- MEI(itemBank, item = i, x = x, theta = theta, 
                        it.given = itemBank[out, ], model=model, method = method, 
                        priorDist = priorDist, priorPar = priorPar, 
    ...
    select <- ifelse(length(keep) == 1, keep, sample(keep,1))
    res <- list(item = select, par = itemBank[select,], info = infos[select], 
                criterion = criterion, randomesque = randomesque)
  }
    if (crit == "MEPV") {
    items <- rep(1, nrow(itemBank))
    ...
    for (i in 1:length(items)) {
      if (items[i] > 0) 
        epvs[i] <- EPV(itemBank, item = i, x = x, theta = theta, 
                       it.given = itemBank[out, ], model=model, priorDist = priorDist, 
                       priorPar = priorPar, D = D, parInt = parInt)
    }
    epVal <- sort(epvs)[min(c(randomesque, sum(items)))]
    keep <- (1:nrow(itemBank))[epvs <= epVal]
    select <- ifelse(length(keep) == 1, keep, sample(keep,1))
    res <- list(item = select, par = itemBank[select,], info = epvs[select], 
                criterion = criterion, randomesque = randomesque)
  }
  if (crit == "random") {
    items <- rep(1, nrow(itemBank))
    ...
    res <- list(item = ind, par = itemBank[ind, ], 
  ...
  if (crit == "progressive") {
    ...
    if (rule=="precision") {
      ...
      if (items_administered > 0) 
        wq <- max(cuminfo/infostop, items_administered/(maxItems-1))^AP
    } 
    if (rule=="length") {
      if (items_administered > 0) {
  ...
  if (crit == "proportional") {
    ...
    if (rule=="precision") {
      ...
      cuminfo <- (1/SETH)^2
      if (items_administered > 0) 
        wq <- infostop * max(cuminfo/infostop,items_administered/(maxItems-1))^AP
    } 
    if (rule=="length") 
      if (items_administered > 0) {
        numerador <- sum((1:items_administered)^AP)
  ...
  if (crit == "thOpt") {
    if (!is.null(model)) stop("'thOpt' rule cannot be considered with polytomous items",call.=FALSE)
    items <- rep(1, nrow(itemBank))
    items[OUT] <- 0
    ...
    thstar <- itemBank[,2] + log((xstar-itemBank[,3]) / (itemBank[,4] - xstar))/ itemBank[,1]
  ...
  if (is.null(cbControl)) 
    ...
    res[[6]] <- empProp
    postProp <- NULL
    for (i in 1:nrGroup) postProp[i] <- length(c(res$item,out)[cbGroup[c(res$item, out)] == cbControl$names[i]])
