startItems<-function (itemBank, model = NULL, fixItems = NULL, seed = NULL, 
    nrItems = 1, theta = 0, D = 1, randomesque = 1, random.seed=NULL, startSelect = "MFI", 
    nAvailable = NULL) 
{
...
if (!is.null(random.seed)) set.seed(random.seed)
                  for (i in 1:length(thStart)) {
                    ...
                    ra <- rank(abs(item.dist - min(item.dist[pos.adm == 
                      1])))
                    prov <- which(ra <= randomesque)[pos.adm[ra <= 
                      randomesque] == 1]
...
if (!is.null(random.seed)) set.seed(random.seed)
                  for (i in 1:length(thStart)) {
                    ...
                    ra <- rank(abs(item.info - max(item.info[pos.adm == 
                      1])))
                    prov <- which(ra <= randomesque)[pos.adm[ra <= 
                      randomesque] == 1]
                    items[i] <- ifelse(length(prov) == 1, prov, 
