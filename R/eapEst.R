eapEst<-function(it,x,D=1,priorDist="norm",priorPar=c(0,1),lower=-4,upper=4,nqp=20){
...
return(res)}
RES<-integrate(g,lower=lower,upper=upper,subdivisions=nqp)$value/integrate(h,lower=lower,upper=upper,subdivisions=nqp)$value
return(RES)}
