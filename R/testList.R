    ...
    if (!is.list(list)) 
        ...
        if (is.null(names(list))) 
            ...
            elements <- switch(type, start = c("fixItems", "seed", 
                "nrItems", "theta", "halfRange", "startSelect"), 
                test = c("method", "priorDist", "priorPar", "range", 
                  "D", "parInt", "itemSelect", "infoType","randomesque"), stop = c("rule", 
                    ...
                    repeat {
                      ...
                      vect <- c(sum(names(list)[i] == intNames),  
                        ...
                        sum(names(list)[i] == eapNames), sum(names(list)[i] == 
                          ...
                          startNames),sum(names(list)[i] == intOnlyNames))
                      ind <- (1:13)[vect == 1]
                      prov <- switch(ind, `1` = ifelse(is.null(list[[i]]), 
                        ...
                        TRUE, ifelse(is.numeric(list[[i]]), ifelse(length(list[[i]]) == 
                          1, TRUE, FALSE), FALSE)), `3` = ifelse(is.numeric(list[[i]]) & 
                        ...
                        `7` = (is.numeric(list[[i]]) & length(list[[i]]) == 
                          ...
                          FALSE & length(list[[i]]) == 1 & sum(list[[i]] == 
                          c("MFI", "Urry", "MLWI", "MPWI", "MEI", 
                            "MEPV", "random")) == 1), `11` = (is.list(list[[i]]) == 
                          FALSE & length(list[[i]]) == 1 & sum(list[[i]] == 
                          c("observed", "Fisher")) == 1), `12` = (is.list(list[[i]]) == 
                          FALSE & length(list[[i]]) == 1 & sum(list[[i]] == 
                          c("bOpt", "MFI")) == 1), `13` = ifelse(is.numeric(list[[i]]), ifelse(max(abs(list[[i]] - 
                      ...
                      if (!prov) {
                        ...
                        res$message <- switch(ind, `1` = paste("element '", 
                          ...
                          `9` = paste("element '", names(list)[i], 
                            ...
                            names(list)[i], "' of '", deparse(substitute(list)), 
                            "' must be either 'MFI', 'Urry',", 
                            "\n", " 'MLWI', 'MPWI', 'MEI', 'MEPV' or 'random'", 
                            sep = ""), `11` = paste("element '", 
                            names(list)[i], "' of '", deparse(substitute(list)), 
                            "' must be either 'observed' or 'Fisher'", 
                            sep = ""), `12` = paste("element '", 
                            names(list)[i], "' of '", deparse(substitute(list)), 
                            "' must be either 'bOpt' or 'MFI'", 
                            sep = ""),`13` = paste("element '", 
                          names(list)[i], "' of '", deparse(substitute(list)), 
                          "' must be a positive integer value",sep=""))
                        break
                      }
                      else {
                        if (i == length(list)) 
